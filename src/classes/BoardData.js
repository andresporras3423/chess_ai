...
class BoardData {
  ...
  getAllAvailableMoves = () => {
    ...
    this.lastMovementCoordinates[0] === "w"
      ? this.game.add_recent_board("black")
      : this.game.add_recent_board("white");
    this.availableMoves = Array.from(this.game.board.movements).map((move) =>
      this.last_movement_reduced(move)
    );
      ...
      row.forEach((cell, j) => {
        if (cell.piece[0] === "b") {
          if (cell.piece === "bk") {
            ...
            this.game.positions.black_pieces[
              `${cell.piece}${list_index[cell.piece]}`
            ] = new Cell(i, j);
  ...
  selectPiece = (cell) => {
    ...
    if (this.selectedPiece === null) {
      ...
      if (
        (this.whitePlaying && cell.piece[0] == "w") ||
        (!this.whitePlaying && cell.piece[0] == "b")
      ) {
  ...
  check_white_en_passant = (cell1, cell2) => {
    ...
    if (
      cell1.y === 3 &&
      capture.includes(`${cell1.y - cell2.y}${cell1.x - cell2.x}`) &&
      this.lastMovement2.piece === "bp" &&
      this.lastMovement2.y === 3 &&
      this.lastMovement2.x === cell2.x &&
      this.lastMovement1.y === 1 &&
      this.lastMovement1.x === cell2.x
    ) {
  ...
  check_black_en_passant = (cell1, cell2) => {
    ...
    if (
      cell1.y === 4 &&
      capture.includes(`${cell1.y - cell2.y}${cell1.x - cell2.x}`) &&
      this.lastMovement2.piece === "wp" &&
      this.lastMovement2.y === 4 &&
      this.lastMovement2.x === cell2.x &&
      this.lastMovement1.y === 6 &&
      this.lastMovement1.x === cell2.x
    ) {
      //in this case, delete the pawn that moves in the previous move
      this.lastMovement2.piece = "";
  ...
  updateBoardAfterValidMove = (cell, newPiece, humanMove = true) => {
    ...
    const moveToCorner = {
      "00": () => (this.blackLongCastling = false),
      "07": () => (this.blackShortCastling = false),
      70: () => (this.whiteLongCastling = false),
      77: () => (this.whiteShortCastling = false),
    };
    ...
    if (this.selectedPiece.piece === "wr" || this.selectedPiece.piece === "br")
      this.update_available_castling(this.selectedPiece);
    else if (
      this.selectedPiece.piece === "wk" ||
      this.selectedPiece.piece === "bk"
    )
      this.check_castling(this.selectedPiece, cell);
    else if (this.selectedPiece.piece === "wp")
      this.check_white_en_passant(this.selectedPiece, cell);
    else if (this.selectedPiece.piece === "bp")
      this.check_black_en_passant(this.selectedPiece, cell);
    ...
    if (this.lastMovement1 !== null)
      this.lastMovement1.removeColor("last-move");
    if (this.lastMovement2 !== null)
      this.lastMovement2.removeColor("last-move");
    ...
    if (humanMove && !this.game.board.game_finished) {
      ...
      this.updateBoardAfterValidMove(
        this.objectCells[computer_move[4]][computer_move[5]],
        computer_move[3],
        false
      );
    }

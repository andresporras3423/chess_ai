...
class ComputerMove {
  ...
  minimax = (game, depth, isComputerMove)=>{
    ...
    if(status==="game started" && depth===0) return this.boardScore(game);
    ...
    game.board.movements_available.forEach((move)=>{
      ...
      const new_score =this.minimax(nGame, this.depth-1, !isComputerMove);
  ...
  boardScore = (game)=>{
    const white_score = Object.keys(game.white_pieces).reduce((t,piece)=>{
    ...
    const black_score = Object.keys(game.black_pieces).reduce((t,piece)=>{
